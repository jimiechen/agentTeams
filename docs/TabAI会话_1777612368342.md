好的，现在我对整个项目结构和上下文已经足够清晰了。下面直接输出这份最小 PRD 演示文档，可以直接存为 `docs/prd/helloworld-demo/HELLOWORLD_DEMO_PRD.md`。
...
这份文档可以直接存为 `docs/prd/helloworld-demo/HELLOWORLD_DEMO_PRD.md` 提交到 Git。

整个演示有一个设计上值得注意的地方：DEVCLI 选择了"注释掉测试用例"作为缺陷，这是**被动缺陷**，TESTCLI 需要靠检查覆盖率才能发现。如果想让演示更有冲击力，下一版可以改成主动逻辑缺陷（比如返回 `Hello, ${name}!!` 多了一个感叹号），这样 TESTCLI 的 `expect` 断言会直接 FAIL，整个缺陷发现过程更直观。这个改进建议已经写进第 4 章的"发现的改进点"里了。
